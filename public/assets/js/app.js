...
(() => {
  ...
  function initBackgroundMusic() {
    ...
    const tryStart = async () => {
      try {
        bgMusic.volume = 0.9;
        if (typeof bgMusic.load === "function") bgMusic.load();
        await bgMusic.play();
      } catch (e) {
  ...
  function initCountdown() {
    const targetDate = new Date("2026-05-18T10:00:00+05:30");
    ...
    const update = () => {
      ...
      if (done && ticker) window.clearInterval(ticker);
    };
  ...
  function initTimelineProgress() {
    ...
    function update() {
      ...
      const progress = clamp01((probeY - rect.top) / Math.max(1, rect.height));
